...
export default function App(){
  ...
  const improveRanked=[...soldiers].filter(u=>(u.bench||u.dead||u.squat)&&(u.prev_bench||u.prev_dead||u.prev_squat)).map(u=>{const cur=(u.bench||0)+(u.dead||0)+(u.squat||0),prev=(u.prev_bench||0)+(u.prev_dead||0)+(u.prev_squat||0);return{...u,improveRate:prev>0?(cur-prev)/prev*100:0,curTotal:cur,prevTotal:prev};}).sort((a,b)=>b.improveRate-a.improveRate);
  ...
  async function doUpdateLifts(){
    ...
    setLifts({bench:"",dead:"",squat:""});setEditId(null);setLoading(false);
  ...
  return(
    ...
    {page==="register"&&(<div style={{minHeight:"100vh",background:C.bg,padding:"40px 24px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><div style={{maxWidth:400,width:"100%"}} className="fu"><div style={{display:"flex",gap:5,marginBottom:32}}>{[1,2,3].map(i=><div key={i} style={{flex:1,height:3,borderRadius:3,background:i<=regStep?C.accent:C.bg3,transition:"background .3s"}}/>)}</div><ErrBanner/>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20}}>
        ...
        <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,marginTop:2}}>
          <div style={{color:C.text2,fontSize:12,marginBottom:8}}>🔒 비밀번호 설정 <span style={{color:C.text3,fontSize:11}}>(다른 기기 로그인 시 사용)</span></div>
    ...
    {page==="lifts"&&(()=>{const user=soldiers.find(u=>u.id===editId);return(<div style={{minHeight:"100vh",background:C.bg,padding:"40px 24px",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{maxWidth:400,width:"100%"}} className="fu"><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:28}}><button onClick={()=>setPage(myId===editId?"landing":"select")} style={{background:"none",border:"none",color:C.text2,fontSize:22,cursor:"pointer"}}>←</button><div><h2 style={{fontFamily:"'Black Han Sans',sans-serif",fontSize:26,color:C.text}}>기록 갱신</h2><p style={{color:C.text2,fontSize:13}}>{user?.name}{user?.streak>0&&<span style={{marginLeft:8,color:"#FB923C"}}>🔥{user.streak}일 연속</span>}</p></div></div><ErrBanner/>{LIFT_LABELS.map(({key,icon,label})=>(<div key={key} style={{marginBottom:12}}><label style={{color:C.text2,fontSize:13,display:"block",marginBottom:6}}>{icon} {label}{user?.[key]&&<span style={{color:C.text3,marginLeft:8,fontSize:11}}>현재 {user[key]}kg</span>}</label><GlowInput placeholder={`${label} 1RM (kg)`} value={lifts[key]} onChange={v=>setLifts(l=>({...l,[key]:v}))} type="number"/></div>))}<div style={{marginTop:8,marginBottom:4}}><ScorePreview bench={lifts.bench} dead={lifts.dead} squat={lifts.squat} weight={user?.weight} height={user?.height}/></div><div style={{display:"flex",gap:10,marginTop:8}}><Ghost onClick={()=>setPage(myId===editId?"landing":"rankings")} style={{flex:1,padding:13}}>취소</Ghost><Btn onClick={()=>{if(lifts.bench&&lifts.dead&&lifts.squat)doUpdateLifts();}} disabled={!lifts.bench||!lifts.dead||!lifts.squat} style={{flex:2,padding:13}}>🔥 갱신하기</Btn></div></div></div>);})()} 

    {page==="select"&&(<div style={{minHeight:"100vh",background:C.bg,padding:"24px 16px"}}><div style={{maxWidth:480,margin:"0 auto"}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20,paddingTop:8}}><button onClick={()=>setPage("rankings")} style={{background:"none",border:"none",color:C.text2,fontSize:22,cursor:"pointer"}}>←</button><h2 style={{fontFamily:"'Black Han Sans',sans-serif",fontSize:22,color:C.text}}>병사 선택</h2></div><div className="fu" style={{display:"flex",flexDirection:"column",gap:6}}>{soldiers.map(u=>{const has=u.bench||u.dead||u.squat,info=has?SCORES[calcScore(u.bench,u.dead,u.squat,u.weight,u.height)]:null;return(<button key={u.id} onClick={()=>{setEditId(u.id);setLifts({bench:u.bench||"",dead:u.dead||"",squat:u.squat||""});setPage("lifts");}} style={{display:"flex",alignItems:"center",gap:10,background:C.bg3,border:`1px solid ${C.border2}`,borderRadius:10,padding:"10px 13px",cursor:"pointer",textAlign:"left"}} onMouseEnter={e=>e.currentTarget.style.borderColor=C.accent+"60"} onMouseLeave={e=>e.currentTarget.style.borderColor=C.border2}>{u.photo?<img src={u.photo} style={{width:40,height:40,borderRadius:"50%",objectFit:"cover",flexShrink:0}}/>:<div style={{width:40,height:40,borderRadius:"50%",background:C.bg4,display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,flexShrink:0}}>👤</div>}<div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}><span style={{fontWeight:900,color:C.text,fontSize:14}}>{u.name}</span><RankBadge r={u.rank_name}/><MosBadge m={u.mos}/></div><div style={{color:C.text2,fontSize:11,marginTop:1}}>{has?`벤치 ${u.bench} · 데드 ${u.dead} · 스쿼트 ${u.squat}`:"기록 없음"}</div></div>{info&&<span style={{fontSize:17}}>{info.emoji}</span>}</button>);})}</div></div></div>)}
    ...
    {showSetupPopup&&(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",zIndex:999,display:"flex",alignItems:"flex-end",justifyContent:"center"}}><div style={{background:"linear-gradient(180deg,#0D1117,#07090C)",border:"1px solid #C8A94250",borderRadius:"16px 16px 0 0",padding:"24px 20px",width:"100%",maxWidth:480,animation:"fadeUp .3s ease"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}><span style={{fontSize:24}}>🔒</span><div><div style={{fontFamily:"'Black Han Sans',sans-serif",fontSize:18,color:C.accent}}>계정 보안 설정</div><div style={{color:C.text3,fontSize:12,marginTop:2}}>비밀번호를 설정하면 다른 기기에서도 로그인할 수 있어요</div></div></div>
