...
export default function App(){
  ...
  const [reg,setReg]=useState({name:"",height:"",weight:"",photo:null,rankName:"",mos:"",unitCode:"",division:"",brigade:"",company:"",post:"",password:""});
  ...
  async function doRegister(){
    ...
    setReg({name:"",height:"",weight:"",photo:null,rankName:"",mos:"",unitCode:"",division:"",brigade:"",company:"",post:"",password:""});
  ...
  return(
    ...
    {/* 다른 기기 로그인 */}
    {page==="login"&&(<div style={{minHeight:"100vh",background:C.bg,padding:"40px 24px",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{maxWidth:400,width:"100%"}} className="fu"><button onClick={()=>setPage("landing")} style={{background:"none",border:"none",color:C.text2,fontSize:22,cursor:"pointer",marginBottom:24}}>←</button><h2 style={{fontFamily:"'Black Han Sans',sans-serif",fontSize:28,color:C.text,marginBottom:4}}>로그인</h2><p style={{color:C.text2,fontSize:14,marginBottom:24}}>이름과 비밀번호로 내 계정을 찾습니다</p><ErrBanner/><div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}><GlowInput placeholder="이름 (등록 시 입력한 이름)" value={loginName} onChange={setLoginName}/><GlowInput placeholder="비밀번호" value={loginPw} onChange={setLoginPw} type="password"/></div><Btn onClick={doLogin} disabled={!loginName.trim()||!loginPw.trim()} style={{width:"100%",padding:14}}>🔑 로그인</Btn></div></div>)}
